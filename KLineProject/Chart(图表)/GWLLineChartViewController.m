...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"折线图2";
...
#pragma mark - 是否绘制图表边框，绘制后就不需要绘制x轴和y轴的轴线了。默认NO
//    When enabled, the borders rectangle will be rendered.启用后，将渲染边框矩形。
//    If this is enabled, there is no point drawing the axis-lines of x- and y-axis.
#pragma mark - 如果启用此选项，则没有绘制x轴和y轴轴线的点。
...
//    设置间隙
    [self.lineChartView setExtraOffsetsWithLeft:0 top:0 right:0 bottom:0];
...
#pragma mark -  !!!: ComponentBase 属性
    
#pragma mark - 是否显示x轴。默认YES
    xAxis.enabled = YES;
    //默认5.0
    xAxis.xOffset = 5.0;
